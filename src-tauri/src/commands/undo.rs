...
#[tauri::command]
pub fn undo_operation(state: State<'_, AppState>) -> Result<UndoRedoState, AppError> {
    let op = {
...
#[tauri::command]
pub fn redo_operation(state: State<'_, AppState>) -> Result<UndoRedoState, AppError> {
    let op = {
        let mut history = state
            .operation_history
            .lock()
    ...
    // For redo, we don't re-apply — the undo already pushed it back.
    // In a full impl, we'd store the "after" state too. For now, just return state.
    let _ = op;
